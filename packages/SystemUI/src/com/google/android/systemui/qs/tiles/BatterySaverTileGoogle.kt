...
constructor(
    ...
    override fun handleUpdateState(state: BooleanState, arg: Any) {
        ...
        if (state.state == Tile.STATE_ACTIVE && isExtremeEnabled) {
            state.secondaryLabel = mContext.getString(KtR.string.extreme_battery_saver_text)
        } else {
    ...
    override fun onExtremeBatterySaverChanged(isExtreme: Boolean) {
        if (isExtremeEnabled != isExtreme) {
            isExtremeEnabled = isExtreme
            refreshState()
        }
    }
}
